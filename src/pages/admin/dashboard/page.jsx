...
export class AdminDashboardView extends Component {
  ...
  async getReport(year) {
    ...
    if (result !== null) {
      this.setState({ dataReportMonth: result }, this.checkData);
    }
  }
  ...
  async getReportCategory(year) {
    ...
    if (result !== null) {
      this.setState({ dataReportCategoryYear: result }, this.checkData);
    }
  }

  async getReportSolve(year, month) {
    const dashboardRepository = new DashboardRepository();
    const result = await dashboardRepository.getReportSolve(year, this.convertMonths(month));
    if (result !== null) {
      this.setState({ dataReportSolve: result }, this.checkData);
    }
  }

  async getReportSpam(year, month) {
    const dashboardRepository = new DashboardRepository();
    const result = await dashboardRepository.getReportSpam(year, this.convertMonths(month));
    if (result !== null) {
      this.setState({ dataReportSpam: result }, this.checkData);
    }
  }
  ...
  async getReportDetail() {
    ...
    if (result !== null) {
      this.setState({ dataReportDetail: result }, this.checkData);
    }
  }
                ...
                <div className="flex-auto w-1/2">
                  <div className="rounded-xl bg-white dark:bg-gray-900 drop-shadow-md w-full">
                    <Dropdown label={this.state.bar1Label === 0 ? "All Year" : this.state.bar1Label} dismissOnClick={false} color="light">
                      {this.state.bar1.map(item => (
                          <Dropdown.Item key={item} onClick={() => this.handleBar1(item)}>
                            {item === 0 ? "All Year" : item}
                          </Dropdown.Item>
                      ))}
                    </Dropdown>
                ...
                <div className="flex-auto w-1/2">
                  <div className="rounded-xl bg-white dark:bg-gray-900 drop-shadow-md">
                    <Dropdown label={this.state.bar2Label === 0 ? "All" : this.state.bar2Label} dismissOnClick={false} color="light">
                      {this.state.bar2.map(item => (
                          <Dropdown.Item key={item} onClick={() => this.handleBar2(item)}>
                            {item === 0 ? "All" : item}
                          </Dropdown.Item>
                      ))}
                    </Dropdown>
                      ...
                      <div className="rounded-xl bg-white dark:bg-gray-900 drop-shadow-md m-2">
                        <div className="flex-auto h-full" style={{ flexBasis: '50%' }}>
                          <Dropdown label={this.state.pie1YearLabel === 0 ? "All" : this.state.pie1YearLabel} dismissOnClick={false} color="light">
                            {this.state.pie1Year.map(item => (
                                <Dropdown.Item key={item} onClick={() => this.handlePie1Year(item)}>
                                  {item === 0 ? "All" : item}
                                </Dropdown.Item>
                            ))}
                          </Dropdown>
                          <Dropdown label={this.state.pie1MonthLabel} dismissOnClick={false} color="light">
                            {this.state.pie1Month.map(item => (
                                <Dropdown.Item key={item} onClick={() => this.handlePie1Month(item)}>
                                  {item}
                                </Dropdown.Item>
                            ))}
                          </Dropdown>
                      ...
                      <div className="rounded-xl bg-white dark:bg-gray-900 drop-shadow-md m-2">
                        <div className="flex-auto h-full" style={{ flexBasis: '50%' }}>
                          <Dropdown label={this.state.pie2YearLabel === 0 ? "All" : this.state.pie2YearLabel} dismissOnClick={false} color="light">
                            {this.state.pie2Year.map(item => (
                                <Dropdown.Item key={item} onClick={() => this.handlePie2Year(item)}>
                                  {item === 0 ? "All" : item}
                                </Dropdown.Item>
                            ))}
                          </Dropdown>
                          <Dropdown label={this.state.pie2MonthLabel} dismissOnClick={false} color="light">
                            {this.state.pie2Month.map(item => (
                                <Dropdown.Item key={item} onClick={() => this.handlePie2Month(item)}>
                                  {item}
                                </Dropdown.Item>
                            ))}
                          </Dropdown>
                      ...
                      {dataReportDetail.reportDetail.map((item, index) => (
                          <div key={index} className="h-full flex items-center justify-center">
                            <Card className="w-full h-full flex flex-col justify-center items-center">
                              ...
                              <div className="text-4xl font-bold font-mono text-purple-800">{item.number}</div>
                            </Card>
                          </div>
                      ))}
                    </div>
